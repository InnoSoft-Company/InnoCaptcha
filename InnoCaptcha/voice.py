...
class VoiceCaptcha():
  ...
  def create(self, phrase=None, ip=None, session_id=None):
    ...
    with DB() as db:
      db.execute("""INSERT INTO voice (id, answer, attempts, ip_address, session_id, created_at, expires_at) VALUES (?, ?, 0, ?, ?, CURRENT_TIMESTAMP, (datetime('now', '+5 minutes')))""", 
                 (self.id, self.phrase, ip, session_id))
      db.commit()
